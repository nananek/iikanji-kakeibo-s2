...
- 証憑画像は base64 で JSON に同梱され、取込時はクライアント（ブラウザ）が全件メモリに展開する。
  写真が大量にある場合は、`--user-id`/期間で対象を絞るか `--storage-dir` を外して財務データだけ先に
  取り込み、証憑は別途分割して取り込むとブラウザのメモリ負荷を避けられる（v1 では逐次 streaming 未対応）。
